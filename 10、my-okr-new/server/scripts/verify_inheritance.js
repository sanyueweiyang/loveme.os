...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const root = yield (0, planService_1.createPlanNode)({ title: '[INHERIT-TEST] Root', level: 1, planCategory: '其他' });
        const l2 = yield (0, planService_1.createPlanNode)({ title: '[INHERIT-TEST] O', level: 2, parentId: root.id, planCategory: '其他' });
        const l3 = yield (0, planService_1.createPlanNode)({ title: '[INHERIT-TEST] KR', level: 3, parentId: l2.id, planCategory: '其他' });
        const l4 = yield (0, planService_1.createPlanNode)({ title: '[INHERIT-TEST] D1', level: 4, parentId: l3.id, planCategory: '其他' });
        const l5 = yield (0, planService_1.createPlanNode)({ title: '[INHERIT-TEST] D2', level: 5, parentId: l4.id, planCategory: '其他' });
        // 3. Setup: Current Month (March) Tasks
        // Unfinished Original Task
        const t1 = yield (0, planService_1.createPlanNode)({
        ...
        yield prisma.planNode.update({ where: { id: t1.id }, data: { monthCode: '03' } });
        // Unfinished Plan Outside Task
        const t2 = yield (0, planService_1.createPlanNode)({
        ...
        yield prisma.planNode.update({ where: { id: t2.id }, data: { monthCode: '03' } });
        // Finished Task (Should not inherit)
        const t3 = yield (0, planService_1.createPlanNode)({
        ...
        yield prisma.planNode.update({ where: { id: t3.id }, data: { monthCode: '03' } });
        // 4. Setup: Next Month (April) Targets
        // Existing Target from Annual Plan
        const t4 = yield (0, planService_1.createPlanNode)({
        ...
        const titles = result.inheritedTasks.map((t) => t.title);
