// api-proxy v6 — uses req.text() to avoid Deno JSON crash
const corsHeaders = {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Cannot read body" }),
  ...
  try {
    const init: RequestInit = { method, headers: { "Content-Type": "application/json", "Bypass-Tunnel-Reminder": "true" } };
    if (bodyPayload && method !== "GET" && method !== "HEAD") {
      init.body = JSON.stringify(bodyPayload);
    }

    const resp = await fetch(`${TARGET_API_URL}${endpoint}`, init);
    ...
    try {
      ...
      return new Response(JSON.stringify({ raw: text }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
  } catch (err: unknown) {
